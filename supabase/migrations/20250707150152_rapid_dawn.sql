/*
  # Add Verified Pro Badge for Restaurants

  1. New Badge Definition
    - Create "Verified Pro" badge for restaurants reviewed by 3+ professional reviewers
    - Icon: 🏅
    - Color: Indigo (#4F46E5)

  2. Award Logic
    - Check for restaurants with reviews from users who have Silver, Gold, or Platinum Reviewer badges
    - Award badge to restaurants with 3+ such reviews

  3. Direct Assignment
    - Award badge to specific restaurant: 20883f2b-8ae2-4f71-b424-88c0b82e014e
    - Check all other restaurants for eligibility
*/
...
DO $$
DECLARE
  target_restaurant_id uuid := '20883f2b-8ae2-4f71-b424-88c0b82e014e';
  restaurant_exists boolean;
  verified_pro_badge_uuid uuid;
BEGIN
  -- Check if restaurant exists
  SELECT EXISTS(SELECT 1 FROM restaurants WHERE id = target_restaurant_id) INTO restaurant_exists;
  
  IF NOT restaurant_exists THEN
    RAISE NOTICE 'Restaurant with ID % does not exist', target_restaurant_id;
    RETURN;
  END IF;
  ...
  INSERT INTO restaurant_badges (restaurant_id, badge_id)
  VALUES (target_restaurant_id, verified_pro_badge_uuid)
  ON CONFLICT (restaurant_id, badge_id) DO NOTHING;
  
  RAISE NOTICE 'Successfully granted Verified Pro badge to restaurant %', target_restaurant_id;
END $$;
